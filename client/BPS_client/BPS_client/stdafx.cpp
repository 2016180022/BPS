...
ID3D12Resource* CreateBufferResource(ID3D12Device* pD3dDevice, ID3D12GraphicsCommandList* pD3dCommandList,
	void* pData, UINT bytes, D3D12_HEAP_TYPE d3dHeapType, D3D12_RESOURCE_STATES d3dResourceStates,
	ID3D12Resource** ppD3dUploadBuffer)
{
	...
	if (pData)
	{
		...
		case D3D12_HEAP_TYPE_DEFAULT:
		{
			if (ppD3dUploadBuffer)
			{
				...
				pD3dDevice->CreateCommittedResource(&d3dHeapPropertiesDesc, D3D12_HEAP_FLAG_NONE,
					&d3dResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, __uuidof(ID3D12Resource),
					(void**)ppD3dUploadBuffer);

				D3D12_RANGE d3dReadRange = { 0,0 };
				UINT8* pBufferDateBegin = NULL;
				(*ppD3dUploadBuffer)->Map(0, &d3dReadRange, (void**)&pBufferDateBegin);
				memcpy(pBufferDateBegin, pData, bytes);
		...
		case D3D12_HEAP_TYPE_UPLOAD:
		{
			...
			memcpy(pBufferDataBegin, pData, bytes);
