...
void Scene::BuildObjects(ID3D12Device* pD3dDevice, ID3D12GraphicsCommandList* pD3dCommandList)
{
	...
	shaderNum = 1;
	ppShaders = new Shader * [shaderNum];
...
void Scene::ReleaseObjects()
{
	...
	if (ppShaders)
	{
		...
		{
			ppShaders[i]->ReleaseShaderVariables();
			ppShaders[i]->ReleaseObjects();
			ppShaders[i]->Release();
		}
		delete[] ppShaders;
	}
...
void Scene::ReleaseUploadBuffers()
{
	if (ppShaders)
	{
		for (int i = 0; i < shaderNum; ++i)
			if (ppShaders[i])
				ppShaders[i]->ReleaseUploadBuffers();
	}
}
...
bool Scene::OnProcesingKeyboardMessage(HWND hwnd, UINT messageID, WPARAM wParam, LPARAM lParam)
{
	return false;
}

bool Scene::ProcessInput(UCHAR* pKeyBuffer)
{
	return false;
}
...
{
	for (int i = 0; i < shaderNum; ++i)
		ppShaders[i]->AnimateObjects(timeElapsed);
}
...
void Scene::Render(ID3D12GraphicsCommandList* pD3dCommandList)
{
	...
	for (int i = 0; i < shaderNum; ++i)
		ppShaders[i]->Render(pD3dCommandList);
}